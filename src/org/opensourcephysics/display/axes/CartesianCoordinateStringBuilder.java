...
public class CartesianCoordinateStringBuilder extends CoordinateStringBuilder {
  ...
  public String getCoordinateString(DrawingPanel panel, MouseEvent e) {
    ...
    if((Math.abs(x)>100)||(Math.abs(x)<0.01)||(Math.abs(y)>100)||(Math.abs(y)<0.01)) {
      if(xLabel!=null) msg = xLabel+scientificFormat.format((float) x);
      if(yLabel!=null) msg += yLabel+scientificFormat.format((float) y);
    } else {
      if(xLabel!=null) msg = xLabel+decimalFormat.format((float) x);
      if(yLabel!=null) msg += yLabel+decimalFormat.format((float) y);
    }
    return msg;
  }
}
