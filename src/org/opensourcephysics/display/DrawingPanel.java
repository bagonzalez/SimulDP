...
public class DrawingPanel extends JPanel implements ActionListener, Renderable {
  ...
  protected MouseController mouseController = new CMController();             // handles the coordinate display on mouse actions
  ...
  protected double xMarginPercentage = 0.0, yMarginPercentage = 0.0;
  ...
  public void setXMarginPercentage(double _percentage) {
    if(xMarginPercentage==_percentage) {
      return;
    }
    xMarginPercentage = _percentage;
    invalidateImage(); // alidImage = false;
  }
  ...
  public void setYMarginPercentage(double _percentage) {
    if(yMarginPercentage==_percentage) {
      return;
    }
  ...
  public void setPreferredMinMax(double xmin, double xmax, double ymin, double ymax, boolean invalidateImage) {
    ...
    yminPreferred = ymin;
    ymaxPreferred = ymax;
    if(invalidateImage) invalidateImage();
  }
  ...
  public void setPreferredMinMaxX(double xmin, double xmax) {
  	autoscaleX = autoscaleXMin = autoscaleXMax = false;
    if(xminPreferred==xmin && xmaxPreferred==xmax) return;
  	if(Double.isNaN(xmin)) {
  		autoscaleXMin = true;
      xmin = xminPreferred;
  	}
  ...
  public void setPreferredMinMaxY(double ymin, double ymax) {
  	autoscaleY = autoscaleYMin = autoscaleYMax = false;
    if(yminPreferred==ymin && ymaxPreferred==ymax) return;
  	if(Double.isNaN(ymin)) {
  		autoscaleYMin = true;
      ymin = yminPreferred;
  	}
  ...
  protected void scaleX(ArrayList<Drawable>  tempList) {
    ...
    if(measurableFound) {
      ...
      if (autoscaleXMin) xminPreferred = newXMin-autoscaleMargin*range;
      if (autoscaleXMax) xmaxPreferred = newXMax+autoscaleMargin*range;
  ...
  protected void scaleY(ArrayList<Drawable>  tempList) {
    ...
    if(measurableFound) {
      ...
      if (autoscaleYMin) yminPreferred = newYMin-autoscaleMargin*range;
      if (autoscaleYMax) ymaxPreferred = newYMax+autoscaleMargin*range;
    } else {                                       // we don't have any measurables
  ...
  public <T extends Drawable> ArrayList<T>  getDrawables(Class<T> type) {
    ...
    for (Drawable d: all) {
    	if (type.isInstance(d)) objects.add(type.cast(d));
    }
    return objects;
  }
  ...
  public <T extends Drawable> ArrayList<T> getObjectOfClass(Class<T> type) {
    ...
    ArrayList<T> objects = new ArrayList<T>();
    for (Drawable d: all) {
    	if (d.getClass()==type) objects.add(type.cast(d));
    }
    return objects;
  }
