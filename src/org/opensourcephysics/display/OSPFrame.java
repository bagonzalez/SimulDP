...
public class OSPFrame extends JFrame implements Hidable, AppFrame {
   ...
   void disposeChildWindows(){
       ...
       while(it.hasNext()) {
          JFrame f = it.next();
          if(!f.isDisplayable()) continue;  // frame has already been disposed
   ...
   public JButton addButton(String methodName, String text, String toolTipText, final Object target) {
      ...
      if(OSPRuntime.translator!=null) {
         text = OSPRuntime.translator.getProperty(target.getClass(), "custom_button."+text, text); //$NON-NLS-1$
         toolTipText = OSPRuntime.translator.getProperty(target.getClass(), "custom_button."+toolTipText, toolTipText); //$NON-NLS-1$
      }
      b.setText(text);
   ...
   class TranslatableButton extends JButton {
      ...
      void refreshGUI() {
         if(OSPRuntime.translator!=null) {
            setText(OSPRuntime.translator.getProperty(target.getClass(), "custom_button."+text, text)); //$NON-NLS-1$
            setToolTipText(OSPRuntime.translator.getProperty(target.getClass(), "custom_button."+tip, tip)); //$NON-NLS-1$
         }
      }
   }
