...
public class Dataset extends AbstractTableModel implements Measurable, LogMeasurable, Data {
  ...
  // visible in drawing panel; only visible dataseta affect autoscale
 ...
 */
  ...
  /**
   * Returns a null list of complex datasets.
   *
   * @return null
   */
  ...
  protected void drawScatterPlot(DrawingPanel drawingPanel, Graphics2D g2) {
    ...
    for(int i = 0;i<index;i++) {
      ...
      if(drawingPanel.isLogScaleX() && xpoints[i]<=0) continue;
      if(drawingPanel.isLogScaleY() && ypoints[i]<=0) continue;
