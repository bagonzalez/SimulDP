...
public class IntegerRaster extends MeasuredImage implements Dimensioned {
  ...
  public IntegerRaster(int _nrow, int _ncol) {
    ...
    BandedSampleModel csm = new BandedSampleModel(DataBuffer.TYPE_BYTE, ncol, nrow, ncol, new int[] {0, 1, 2},
      new int[] {0,
                 0, 0});
  ...
  public void draw(DrawingPanel panel, Graphics g) {
    if(scaleFactor<1) {
      g.drawImage(image.getScaledInstance(
    		  (int) (scaleFactor*image.getWidth()),(int) (scaleFactor*image.getHeight()),java.awt.Image.SCALE_REPLICATE),
    		  panel.getLeftGutter(),
    		  panel.getTopGutter(),
              panel);
    } else {
