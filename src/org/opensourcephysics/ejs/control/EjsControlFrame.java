...
public class EjsControlFrame extends ParsedEjsControl implements RootPaneContainer, MainFrame {
  ...
  private void createMenuBar() {
    ...
    logToFileItem.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        if(OSPRuntime.appletMode||(OSPRuntime.applet!=null))return;  // applets cannot log to file
  ...
  protected JMenu loadToolsMenu() {
    ...
    if(OSPRuntime.loadDataTool)try {
    ...
    if(OSPRuntime.loadVideoTool)try {
  ...
  protected void loadDisplayMenu() {
    ...
    translateItem.setEnabled(OSPRuntime.translator!=null);
    ...
    final JCheckBoxMenuItem shapeAliasItem =
      new JCheckBoxMenuItem(EjsRes.getString("EjsControlFrame.Drawing_check_box"), false); //$NON-NLS-1$
