...
public class Launcher {
  ...
  static {
    ...
    ResourceLoader.addExtractExtension("pdf"); //$NON-NLS-1$
  }
      ...
      public void stateChanged(ChangeEvent e) {
        ...
        if((previousNode!=null)&&(selectedNode!=null)) {
          ...
          if((selectedNode.htmlURL==null)&&!selectedNode.htmlData.isEmpty()) {
            ...
            LaunchNode.HTML htmlData = selectedNode.htmlData.get(page);
  ...
  protected Launcher setLookAndFeel(final String lf, final boolean always) {
    ...
    if(!always && newDecorations == currentDecorations
    	&& currentLF.getClass().getName().equals(lfType)) {
      return null;
    }
    if(!isVisible()) {
      frame.addWindowListener(new WindowAdapter() {
...
//          	// determine whether link will be opened externally
//          	boolean browseExternally = true;
//          	if (browseExternally) {
//          		browseExternally = !e.getDescription().startsWith("#"); //$NON-NLS-1$
//            	if (browseExternally) {
//		          	for (Enumeration<?> en = e.getSourceElement().getAttributes().getAttributeNames(); en.hasMoreElements();) {
//		          		Object key = en.nextElement();
//		          		if (key.toString().equals("a")) { //$NON-NLS-1$
//			          		String val = e.getSourceElement().getAttributes().getAttribute(key).toString();
//		          			String target = ""; //$NON-NLS-1$
//			          		int n = val.indexOf("target="); //$NON-NLS-1$
//			          		if (n > -1) {
//			          			target = val.substring(n+7);
//			          			browseExternally = !target.startsWith("_self"); //$NON-NLS-1$
//		            		}
//		          		}                		
//		          	}
//            	}
//          	}
          	// browse web-hosted links and pdf files externally
          	boolean browseExternally = !url.getHost().equals("")  //$NON-NLS-1$
      					|| path.endsWith(".pdf"); //$NON-NLS-1$
            if (browseExternally) {
            	if (path.endsWith(".pdf") && path.indexOf("jar!")>-1) { //$NON-NLS-1$ //$NON-NLS-2$
            		// look to see if file exists outside jar
  ...
  private class LauncherFrame extends JFrame {
    ...
    public LauncherFrame() {
      ...
      //setIconImage(ResourceLoader.getImage(OSPRuntime.OSP_ICON_FILE));  // removed by W. Christian
...
////                 com.Ostermiller.util.Browser.displayURL(e.getURL().toString());
//              }
//            }
