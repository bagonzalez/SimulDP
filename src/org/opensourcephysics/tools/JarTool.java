...
public class JarTool implements Tool, Runnable {
  ...
  // --- End of localization
   ...
   private static Map<String, Map<String, ZipEntry>> jarContents 
   		= new HashMap<String, Map<String, ZipEntry>>(); // added by D Brown 2007-10-31
...
// -----------------------------------
   ...
   public File create(ArrayList<String> sources, File parent, File target, Manifest manifest) {
     ...
     try {
       ...
       else chooser.setSelectedFile(new File("default.jar")); //$NON-NLS-1$
       String targetName = OSPRuntime.chooseFilename(chooser);
       if (targetName==null) return null;
       if (! (targetName.toLowerCase().endsWith(".jar") || //$NON-NLS-1$
              targetName.toLowerCase().endsWith(".zip")) ) targetName = targetName + ".jar"; //$NON-NLS-1$ //$NON-NLS-2$
    ...
    static public boolean compress (ArrayList<File> sources, File target, Manifest manifest) {
      try {
        ...
        for (Iterator<File> it=sources.iterator(); it.hasNext(); ){
          ...
          if (isJar && (manifest!=null) && fileOrDir.getName().equals("META-INF")) continue; // Ignore any existing META-INF directory //$NON-NLS-1$
...
// -----------------------------------
  ...
  static public int confirmOverwrite(String filename) {
    ...
    java.awt.event.MouseAdapter mouseListener =  new java.awt.event.MouseAdapter () {
      public void mousePressed (java.awt.event.MouseEvent evt) {
        ...
        if      (aCmd.equals("yes"))      returnValue.value = YES; //$NON-NLS-1$
        else if (aCmd.equals("no"))       returnValue.value = NO; //$NON-NLS-1$
        else if (aCmd.equals("yesToAll")) returnValue.value = YES_TO_ALL; //$NON-NLS-1$
        else if (aCmd.equals("noToAll"))  returnValue.value = NO_TO_ALL; //$NON-NLS-1$
  ...
  static private StringBuffer processFile (String filename, File file, File targetDirectory, OverwriteValue policy) {
    ...
    while (filename.startsWith("../")) filename = filename.substring(3); //$NON-NLS-1$
