  	...
  	for (int j = 0; j < functions.length; j++) {
  		for (int i = 0; i < functions[j].getParameterCount(); i++) {
  			if (paramEditor.getObject(functions[j].getParameterName(i)) == null) {
    			...
    			paramEditor.addObject(param, j, false, false);
  			}
  		}
  	}
 		refreshFunctions();
...
//__________________________ static methods ___________________________
  ...
  static class Loader implements XML.ObjectLoader {
    ...
    public Object createObject(XMLControl control){
    	UserFunctionEditor editor = (UserFunctionEditor)
    			control.getObject("function_editor"); //$NON-NLS-1$
