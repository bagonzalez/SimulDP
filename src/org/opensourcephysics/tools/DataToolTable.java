...
public class DataToolTable extends DataTable {
  ...
  public DataToolTable(DataToolTab tab) {
    ...
    cantPasteCellsAction = new AbstractAction() {
      public void actionPerformed(ActionEvent e) {
        JOptionPane.showMessageDialog(dataToolTab.dataTool,
    ...
    getPasteDataAction = new AbstractAction() {
      public void actionPerformed(ActionEvent e) {
        ...
        String dataString = dataToolTab.dataTool.paste();
        ArrayList<String> colNames = getSelectedColumnNames();
        if(dataString!=null) {
          pasteData = dataToolTab.dataTool.importCharDelimitedData(dataString, null);
          if(pasteData!=null) {
      ...
      public void mouseClicked(MouseEvent e) {
        ...
        if(OSPRuntime.isPopupTrigger(e)) {
          ...
          if((cols.size()==1)&&dataToolTab.userEditable) {
            ...
            renameColumnItem.addActionListener(new ActionListener() {
              public void actionPerformed(ActionEvent e) {
                ...
                Object input = JOptionPane.showInputDialog(dataToolTab.dataTool,
          ...
          copyColumnsItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              dataToolTab.dataTool.copyDataItem.doClick();
            }
          ...
          FontSizer.setFonts(popup, dataToolTab.dataTool.fontLevel);
          ...
          // label cell clicked: set lead row and show row popup
            ...
            copyRowsItem.addActionListener(new ActionListener() {
              public void actionPerformed(ActionEvent e) {
                ...
                String data = dataToolTab.dataTool.getSelectedTableData(dataToolTab.getName(),
                  DataToolTable.this).toString();
                dataToolTab.dataTool.copy(data);
              }
  ...
  protected WorkingDataset getWorkingData(String colName) {
    ...
    int labelCol = convertColumnIndexToView(0);
    String xName = getColumnName((labelCol==0)
                                 ? 1
                                 : 0);
  ...
  protected HighlightableDataset getSelectedData() {
    ...
    for(int i = 0; i<x.length; i++) {
      ...
      //      System.out.println("putting "+workingIndex+" -> "+i);
    ...
    if(!colSelected||(getSelectedRowCount()==0)) { // nothing selected
      ...
      while(it.hasNext()) {
        ...
        xValues[i] = (row>=x.length)
                     ? Double.NaN
                     : x[row];
        yValues[i] = (row>=y.length)
                     ? Double.NaN
                     : y[row];
        i++;
   ...
   * If the dataset exists, this moves it to the specified column.
   *
  ...
  protected void insertColumn(Dataset data, int col) {
    ...
    for(int i = 0; i<datasets.size(); i++) {
      if(data.getID()==datasets.get(i).getID()) {
        foundData = true;
        dataModelCol = i+1;
        dataViewCol = convertColumnIndexToView(dataModelCol);
        break;
      }
    }
    ...
    int[] modelColumns = new int[foundData
                                 ? len
                                 : len+1];
    ...
    //int kk = convertColumnIndexToView(modelColumns.length);
    refreshTable();
  ...
  protected HashMap<String, double[]> insertCells(int[] rows, HashMap<String, double[]> values) {
    ...
    int trimLength = count-1;
    int[] fillRows = new int[rows.length];
    for(int i = 0; i<rows.length; i++) {
      fillRows[i] = count+i;
      trimLength = Math.max(trimLength, rows[i]);
    }
  ...
  protected boolean isEmptyRow(int row) {
    ...
    while(it.hasNext()) {
      ...
      empty = empty&&Double.isNaN(data.getYPoints()[row]);
    }
    return empty;
  }
  ...
  class HeaderRenderer implements TableCellRenderer {
    ...
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
      ...
      textLine.setText(value.toString());
      ...
      textLine.setFont((sortCol!=convertColumnIndexToModel(col))
                       ? font
      ...
      textLine.setColor(selected
                        ? selectedFG
                        : comp.getForeground());
      textLine.setBackground(selected
                             ? selectedBG
                             : comp.getBackground());
  ...
  class WorkingDataset extends HighlightableDataset {
    ...
    public WorkingDataset(Dataset yDataset) {
      yData = yDataset;
      setColor(yData.getEdgeColor(), yData.getLineColor());
  ...
  protected class TableEdit extends AbstractUndoableEdit {
    ...
    public TableEdit(int type, String colName, Object target, Object value) {
      ...
      this.target = target;
      this.value = value;
      String name = (colName==null)
                    ? null
                    : ": column \""+colName+"\""; //$NON-NLS-1$ //$NON-NLS-2$
