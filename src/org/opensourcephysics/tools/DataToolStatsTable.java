...
public class DataToolStatsTable extends JTable {
  ...
  private Object[] getStatistics(double[] data) {
    ...
    return new Object[] {
        new Double(max), new Double(min), new Double(mean), 
        new Double(sd), new Double(sd/Math.sqrt(count)), 
        new Integer(count)};
  }
  ...
  public void refreshStatistics() {
    ...
    for (int j = 1; j < model.getColumnCount(); j++) {
	    ...
	    for (int i = 0; i < model.getRowCount(); i++) {
	    	...
	    	if (val == null) val = new Double(Double.NaN);
	    	datalist.add(val);
	    }
  ...
  class NumberRenderer extends JLabel implements TableCellRenderer {
    ...
    public Component getTableCellRendererComponent(JTable table, Object value,
                                                   ...
                                                   int column) {
    	if (font == null) {
    		font = getDefaultRenderer(String.class).getTableCellRendererComponent(
          			DataToolStatsTable.this, "", false, false, 0, 0).getFont(); //$NON-NLS-1$
    	}
      setFont(font);
      ...
      else setText(format.format(value));
      return this;
    }
  }
