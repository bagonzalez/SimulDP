...
public class ImageVideo extends VideoAdapter {
  ...
  public ImageVideo(Image image) {
    if (image != null) insert(new Image[] {image}, 0, null);
  }
  ...
  public void insert(String imageName, int index) throws IOException {
    Object[] array = loadImages(imageName,
  ...
  public boolean saveInvalidImages() {
  	...
  	int approved = JOptionPane.showConfirmDialog(
        null,
    ...
    if (approved == JOptionPane.YES_OPTION) {
    	try {
      	...
      	String fileName = file.getAbsolutePath();
  			String[] pathArray = recorder.saveImages(fileName, images);
  			int j = 0;
  	  	for (int i = 0; i < paths.length; i++) {
  	  		if (paths[i].equals("")) { //$NON-NLS-1$
  	  			paths[i] = XML.getRelativePath(pathArray[j++]);
  	  		}
  	  	}
  	  	return true;
...
//_______________________ private/protected methods ____________________________
   ...
   * @throws IOException
   */
  private Object[] loadImages(String imagePath,
                             boolean alwaysAsk,
    ...
    if (!alwaysAsk && !sequence) {
    	...
    	String[] paths = new String[] {XML.getRelativePath(imagePath)};
      return new Object[] {images, paths};
    }
    ArrayList<String> pathList = new ArrayList<String>();
    pathList.add(XML.getRelativePath(imagePath));
    ...
    if (digits == 0) { // no number found, so load single image
    	Image[] images = new Image[] {image};
    	String[] paths = new String[] {XML.getRelativePath(imagePath+extension)};
      return new Object[] {images, paths};
     }
      ...
      while (n < limit-1) {
        ...
        if (!asked && alwaysAsk) {
          ...
          int response = JOptionPane.showOptionDialog(
              null,
              "\""+name + "\" " + MediaRes.getString("ImageVideo.Dialog.LoadSequence.Message") + XML.NEW_LINE + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
  ...
  protected String[] getValidPaths() {
  	ArrayList<String> pathList = new ArrayList<String>();
  	for (int i = 0; i < paths.length; i++) {
  		if (!paths[i].equals("")) pathList.add(paths[i]); //$NON-NLS-1$
  	}
  ...
  protected void insert(Image[] newImages, int index, String[] imagePaths) {
    ...
    for (int i = 0; i < newImages.length; i++) {
    	...
    	if (im instanceof BufferedImage) buf[i] = (BufferedImage)im;
    	else {
